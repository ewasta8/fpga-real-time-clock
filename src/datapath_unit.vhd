...
begin
    ...
    ce_min_jedn  <= '1' when (q_s_dz = x"5" and ce_sek_dzies = '1') else '0';
    ce_godz_jedn <= '1' when (q_m_dz = x"5" and ce_min_dzies = '1') else '0';

    Sek_Jedn:   entity work.d_cntr4ceo port map(clk, rst, tick_1Hz, save_shadow, sh_s_j, open, ce_sek_dzies, q_s_j);
    Sek_Dzies:  entity work.d_cntr4ceo port map(clk, rst_sek_dzies, ce_sek_dzies, save_shadow, sh_s_dz, open, open, q_s_dz);
    Min_Jedn:   entity work.d_cntr4ceo port map(clk, rst, ce_min_jedn, save_shadow, sh_m_j, open, ce_min_dzies, q_m_j);
    Min_Dzies:  entity work.d_cntr4ceo port map(clk, rst_min_dzies, ce_min_dzies, save_shadow, sh_m_dz, open, open, q_m_dz);
    ...
    MUX_H_DZ: entity work.mux4x1 
        port map(d0 => q_h_dz, d1 => sh_h_dz, d2 => sh_al_h_dz, d3 => x"0", s0 => mux_sel(0), s1 => mux_sel(1), y => disp_h_dz);
        
    MUX_H_J:  entity work.mux4x1 
        port map(d0 => q_h_j,  d1 => sh_h_j,  d2 => sh_al_h_j,  d3 => x"0", s0 => mux_sel(0), s1 => mux_sel(1), y => disp_h_j);
        
    MUX_M_DZ: entity work.mux4x1 
        port map(d0 => q_m_dz, d1 => sh_m_dz, d2 => sh_al_m_dz, d3 => x"0", s0 => mux_sel(0), s1 => mux_sel(1), y => disp_m_dz);
        
    MUX_M_J:  entity work.mux4x1 
        port map(d0 => q_m_j,  d1 => sh_m_j,  d2 => sh_al_m_j,  d3 => x"0", s0 => mux_sel(0), s1 => mux_sel(1), y => disp_m_j);
        
    MUX_S_DZ: entity work.mux4x1 
        port map(d0 => q_s_dz, d1 => sh_s_dz, d2 => sh_al_s_dz, d3 => x"0", s0 => mux_sel(0), s1 => mux_sel(1), y => disp_s_dz);
        
    MUX_S_J:  entity work.mux4x1 
        port map(d0 => q_s_j,  d1 => sh_s_j,  d2 => sh_al_s_j,  d3 => x"0", s0 => mux_sel(0), s1 => mux_sel(1), y => disp_s_j);
